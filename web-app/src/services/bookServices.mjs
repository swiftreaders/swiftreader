...
class BookService {
    ...
    static async getBookById(id) {
        ...
        try {
            ...
            const book_content = data.formats["text/plain; charset=us-ascii"] ?? "NOT_FOUND";
            if (book_content == "NOT_FOUND") {
                return;
            }
            console.log("book utf8 url: " + book_content);
            const text_data = await fetch(book_content);
    ...
    static async getParagraphFromText(text) {
        ...
        const result = await model.generateContent("hey how u doing u wasteman ur acc useless work next time");
...
(async () => {
    try {
        // const bookId = 123; // Replace with the desired book ID
        // const id = await BookService.getBookByTopic("nature");
